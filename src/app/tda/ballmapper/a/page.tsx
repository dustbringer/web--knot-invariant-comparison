...
const optionsBM: { [name: string]: string } = {
  ...
  ["b1-a"]: "B1-a",
  // ["bnvdv-3-15-e=0.15-a"]: "BV-3-15-a",
  ["bnvdv-a"]: "BV-a",
  ["hfk2-a"]: "HFK2-a",
  ["hfk2-t1-a"]: "HFK2T1-a",
  ["homflypt-e=0.5-a"]: "HOMFLYPT-a",
  ["jones-a"]: "Jones-a",
  ["khovanov-a"]: "Khovanov-a",
...
export default function BallmapperPage() {
  ...
  const curKnotType = React.useRef<string>("a"); // for invariant bmCmpInv

  const [bmInv, setBmInv] = React.useState<string>("b1-a");
  ...
  const [bmCmpInv, setBmCmpInv] = React.useState<string>("jones-a");
  ...
  React.useEffect(() => {
    if (savedKnotTypes.current.length === 0) {
      ...
      fetch(staticify(`/static/ballmapper/types-3-16.out`))
        .then((res) => res.text())
        .then((res) => {
    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmInv] === undefined) {
          ...
          return Promise.all([
            fetch(staticify(`/static/ballmapper/a/bm-${bmInv}.edge.out`)),
            fetch(staticify(`/static/ballmapper/a/bm-${bmInv}.pcbl.out`)),
          ])
    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmCmpInv] === undefined) {
          ...
          return Promise.all([
            fetch(staticify(`/static/ballmapper/a/bm-${bmCmpInv}.edge.out`)),
            fetch(staticify(`/static/ballmapper/a/bm-${bmCmpInv}.pcbl.out`)),
          ])
            .then((res) => Promise.all(res.map((r) => r.text())))
            .then((res) => {
      ...
      .then((data: { edge: string; pcbl: string }) => {
        ...
        // curKnotType.current = bmCmpInv[bmCmpInv.length - 1]; // last letter of internal name, only needed when mixing a and n
      });
    setCurColorType("");
    setCurColorName("");
  }, [bmCmpInv]);
    ...
    <Container>
      ...
      <div>
        <Typography variant="body1" gutterBottom>
          ...
          <Link href="/bm/a" inPlace sx={{ color: "secondary.main" }}>
            (alternating)
          </Link>{" "}
      ...
      <div>
        ...
        <ul style={{ margin: "0", marginBottom: "1em" }}>
          <li>
            The ballmappers use the same <TeX>\epsilon</TeX> as in the{" "}
            <Link href="/bm" inPlace>
              main ballmapper page
            </Link>
            .
          </li>
          <li>
        ...
        <li>
          ...
          {Object.entries(optionsBM).map((option, i) => (
            <React.Fragment key={`frag ${option[0]}`}>
              <Link
                href={`/static/ballmapper/a/bm-${option[0]}.pcbl.out`}
                key={option[0]}
              >
                {option[1]}
              </Link>
