    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmInv] === undefined) {
          ...
          return Promise.all([
            fetch(staticify(`/static/ballmapper/bm-${bmInv}.edge.out`)),
            fetch(staticify(`/static/ballmapper/bm-${bmInv}.pcbl.out`)),
          ])
    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmCmpInv] === undefined) {
          ...
          return Promise.all([
            fetch(staticify(`/static/ballmapper/bm-${bmCmpInv}.edge.out`)),
            fetch(staticify(`/static/ballmapper/bm-${bmCmpInv}.pcbl.out`)),
          ])
            .then((res) => Promise.all(res.map((r) => r.text())))
            .then((res) => {
    ...
    <Container>
      ...
      <Box sx={{ marginBottom: "10px", overflow: "hidden" }}>
        ...
        <Accordion title="Epsilon movie">
          ...
          ,{" "}
          <Link href={`/static/ballmapper/movie/jones-shrink.gif`} inPlace>
            shrinking
          </Link>
        </Accordion>
      </Box>
