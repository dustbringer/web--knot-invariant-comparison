...
export default function BallmapperPage() {
  ...
  const [bmCmpInv, setBmCmpInv] = React.useState<string>("jones-n");
    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmInv] === undefined) {
          ...
          return Promise.all([
            fetch(staticify(`/static/ballmapper/n/bm-${bmInv}.edge.out`)),
            fetch(staticify(`/static/ballmapper/n/bm-${bmInv}.pcbl.out`)),
          ])
    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmCmpInv] === undefined) {
          ...
          return Promise.all([
            fetch(staticify(`/static/ballmapper/n/bm-${bmCmpInv}.edge.out`)),
            fetch(staticify(`/static/ballmapper/n/bm-${bmCmpInv}.pcbl.out`)),
          ])
            .then((res) => Promise.all(res.map((r) => r.text())))
            .then((res) => {
    ...
    <Container>
      ...
      <div>
        <Typography variant="body1" gutterBottom>
          ...
          <Link href="/bm/n" inPlace sx={{ color: "secondary.main" }}>
            (non-alternating)
          </Link>
        </Typography>
      </div>
        ...
        <li>
          ...
          {Object.entries(optionsBM).map((option, i) => (
            <React.Fragment key={`frag ${option[0]}`}>
              <Link
                href={`/static/ballmapper/n/bm-${option[0]}.pcbl.out`}
                key={option[0]}
              >
                {option[1]}
              </Link>
