    ...
    Promise.resolve()
      .then(() => {
        if (bmSaved.current[bmInv] === undefined) {
          ...
          return Promise.all([
            fetch(staticify(`/static/bm/a/bm-${bmInv}.edge.out`)),
            fetch(staticify(`/static/bm/a/bm-${bmInv}.pcbl.out`)),
          ])
    ...
    Promise.resolve()
      .then(() => {
        if (bmSaved.current[bmCmpInv] === undefined) {
          ...
          return Promise.all([
            fetch(staticify(`/static/bm/a/bm-${bmCmpInv}.edge.out`)),
            fetch(staticify(`/static/bm/a/bm-${bmCmpInv}.pcbl.out`)),
          ])
            .then((res) => Promise.all(res.map((r) => r.text())))
            .then((res) => {
        ...
        <li>
          ...
          {Object.entries(optionsBM).map((option, i) => (
            <React.Fragment key={`frag ${option[0]}`}>
              <Link
                href={`/static/bm/a/bm-${option[0]}.pcbl.out`}
                key={option[0]}
              >
                {option[1]}
              </Link>
